...
def _is_plugin_install(path: Path) -> bool:
    """True when `path` is a host plugin cache / unpacked plugin install.

    Walking ancestors from that tree reaches $HOME and can pick a random
    `*Memory` vault that is not the user's project.
    """
    resolved = Path(os.path.abspath(path))
    text = str(resolved).replace("\\", "/")
    if "/.cursor/plugins/" in text or "/.claude/plugins/" in text:
        return True
    return (
        (resolved / "memory_graph").is_dir()
        and (resolved / "pyproject.toml").is_file()
        and any(
            (resolved / marker).is_dir()
            for marker in (".cursor-plugin", ".claude-plugin", ".codex-plugin")
        )
    )


def _current_project_dir() -> str | None:
    """Host project dir, never a plugin-install cache.

    Order: usable ``CLAUDE_PROJECT_DIR`` → usable ``CURSOR_PROJECT_DIR`` →
    cwd if it is not a plugin install. Plugin-install paths are skipped so
    callers do not walk ancestors from the cache.
    """
    for name in ("CLAUDE_PROJECT_DIR", "CURSOR_PROJECT_DIR"):
        value = _usable_env(name)
        if value and not _is_plugin_install(Path(value)):
            return value
    cwd = os.getcwd()
    if Path(cwd).exists() and not _is_plugin_install(Path(cwd)):
        return cwd
    return None
...
def resolve_memory_dir(explicit: str | None = None, *, required: bool = True) -> str | None:
    ...
    Resolution order:
      ...
      3. Walk ancestors from the project dir up to `$HOME` (inclusive):
         ...
         b. `find_vault(level)` — the level itself or an immediate child
            containing `.obsidian/`.
         First hit wins (closest to the project). A plugin-install path
         (host cache or unpacked plugin) is never used as the walk start.
    ...
    # Never walk ancestors from a plugin-install cwd/cache — that climb
    # reaches $HOME and can index a random *Memory vault.
    ...
    display_dir = project_dir or os.getcwd()
    legacy = _claude_project_root(display_dir) / "memory"
    ...
    raise FileNotFoundError(
        ...
        f"  No Obsidian vault (a directory containing .obsidian/) found in: {display_dir}\n"
...
def resolve_db_path(explicit: str | None = None) -> str:
    ...
    project_dir = _current_project_dir() or os.getcwd()
